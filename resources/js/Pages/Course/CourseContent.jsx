...
export default function CourseContentForm({ message, auth }) {
    ...
    const handleSubmit = (e) => {
        ...
        Inertia.post(route('coursecontent.upload', { courseCode: courseCode }), formData);
    };
                                                ...
                                                {courseLearningOutcomes.map((outcome, index) => (
                                                    <>
                                                        <label key={index} className="inline-flex items-center">
                                                            <input type="checkbox" className="form-checkbox" value={outcome.CLO_ID} onChange={(e) => e.target.checked ? setMappings([...mappings, e.target.value]) : setMappings(mappings.filter(strategy => strategy !== e.target.value))} />
                                            ...
                                            {courseContents && courseContents.length > 0 ? (
                                                courseContents.map((contents, index) => (
                                                    <tr key={index} className={index % 2 === 0 ? 'bg-gray-50 dark:bg-gray-800' : 'bg-white'}>
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-400">{contents.mapping}</td>
                                                    </tr>
                                                ))
                                            ) : (
                                                <tr>
